...
export default function LeavesPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Load HR pending count
      if (showHRSection) {
        try {
          const hrData = await leavesService.getAllLeaveRequestsForHR('PENDING_HR', 1, 1);
      ...
      <section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <Link href="/leaves/balance"
            className="bg-white rounded-lg shadow p-4 text-center hover:shadow-md transition border border-gray-100">
            <div className="text-3xl mb-2">💰</div>
            <div className="text-sm font-medium text-gray-700">View Balances</div>
          </Link>
        ...
        <section className="border-t pt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Link href="/leaves/approvals"
              ...
              <p className="text-sm text-orange-100 mt-4">Click to review team requests</p>
            </Link>
            ...
            <Link href="/leaves/delegation"
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Delegate approvals when away</p>
                </div>
              </div>
            </Link>
            ...
            <div className="bg-white rounded-lg shadow p-6 border border-gray-100">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">View team leave calendar</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {showHRSection && (
        <section className="border-t pt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            <Link href="/leaves/admin/requests"
          ...
          {/* Admin Links */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Link href="/leaves/admin/categories"
              className="bg-white rounded-lg shadow p-4 text-center hover:shadow-md transition border border-gray-100">
              <div className="text-3xl mb-2">📁</div>
              <div className="text-sm font-medium text-gray-700">Categories</div>
            </Link>
            <Link href="/leaves/admin/types"
              className="bg-white rounded-lg shadow p-4 text-center hover:shadow-md transition border border-gray-100">
              <div className="text-3xl mb-2">🏷️</div>
              <div className="text-sm font-medium text-gray-700">Leave Types</div>
            </Link>
            <Link href="/leaves/admin/calendar"
              className="bg-white rounded-lg shadow p-4 text-center hover:shadow-md transition border border-gray-100">
              <div className="text-3xl mb-2">🗓️</div>
              <div className="text-sm font-medium text-gray-700">Calendar</div>
            </Link>
            <Link href="/leaves/admin/entitlements"
              className="bg-white rounded-lg shadow p-4 text-center hover:shadow-md transition border border-gray-100">
              <div className="text-3xl mb-2">⚖️</div>
              <div className="text-sm font-medium text-gray-700">Entitlements</div>
            </Link>
          </div>
        </section>
      )}
